...
// プラグイン定数
define('ANDW_FAQ_SCHEMA_VERSION', '0.2.0');
...
function andw_faq_schema_output_jsonld()
{
	...
	// JSON-LD出力の無効化（オプション設定またはフィルター）
	$disable_jsonld = get_option('andw_faq_schema_disable_jsonld', false);
	if ($disable_jsonld || !apply_filters('andw_faq_schema_enabled', true, $post)) {
		return;
	}
